...
const Zipcode = ({ onLoading, onLocations }) => {
    ...
    return (
        <div>
            <form onSubmit={handleSubmit} className="m-4 flex pt-2">
                <input type="text" value={zipcode} onChange={handleChange} maxLength="5" className="focus:outline-none rounded-l-lg p-2 border-t mr-0 border-b border-l text-gray-800 border-gray-200 bg-white" placeholder="Zipcode" />
                <button className="z-30 px-4 rounded-r-lg bg-white  text-gray-800  border-gray-200 border-t border-b border-r focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 transform hover:scale-110 transition duration-200 ease-in-out" viewBox="0 0 20 20" fill="currentColor">
